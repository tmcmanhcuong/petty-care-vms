<template>
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white border border-gray-200/60 rounded-[10px] shadow-lg w-[512px] relative p-6">
      <!-- Confirmation State -->
      <div v-if="!hasRelatedReceipts" class="flex flex-col gap-4">
        <!-- Header -->
        <div class="flex flex-col gap-2">
          <div class="flex items-start gap-2">
            <span class="text-2xl leading-8">🗑️</span>
        ...
        <div class="flex items-center justify-end gap-2 pt-4">
          ...
          <button
            @click="handleDelete"
            class="bg-[#e7000b] rounded-lg h-9 px-4 py-2 hover:bg-[#c0000a] transition-colors"
          >
            <span class="font-nunito font-medium text-sm leading-5 text-white tracking-tight">Xóa</span>
          </button>
        </div>
      </div>
      ...
      <div v-else class="flex flex-col gap-4">
        ...
        <div class="flex flex-col gap-2">
          <div class="flex items-start gap-2">
            <span class="text-2xl leading-8">⛔</span>
            <h2 class="font-nunito font-semibold text-lg leading-7 text-neutral-950 tracking-tight">
          ...
          <div class="flex flex-col gap-3">
            ...
            <div class="bg-red-50 border border-[#ffc9c9] rounded-[10px] p-3 flex flex-col gap-2">
              ...
              <ul class="list-none space-y-1">
                ...
                >
                  • {{ receipt.code }} - {{ receipt.date }} - {{ formatCurrency(receipt.amount) }}
                </li>
              </ul>
            </div>
        ...
        <div class="flex items-center justify-end gap-2 pt-4">
          ...
          <button
            ...
            class="bg-[#e7000b] rounded-lg h-9 px-4 py-2 hover:bg-[#c0000a] transition-colors"
          >
            <span class="font-nunito font-medium text-sm leading-5 text-white tracking-tight">Ngừng hợp tác</span>
          </button>
        </div>
      </div>
    </div>
